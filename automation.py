...
async def _send_lounge_msg(session, token, person_id, raw_msg) -> bool:
    """
    Sends lounge message(s). Splits by comma and sends each part separately.
    Matches lounge.py behaviour exactly.
    """
    if not raw_msg:
        return False
    ...
    for i, msg_part in enumerate(messages):
        try:
            ...
            logger.error(f"Lounge send error for {person_id} part {i+1}: {e}")
...
async def _send_chatroom_msg(session, token, person_id, raw_msg):
    """
    Opens chatroom (once) then sends message(s) split by comma.
    Matches lounge.py open_chatroom_and_send behaviour exactly.
    Returns True on success, False on failure, "DISABLED" if user has chat disabled.
    """
    if not raw_msg:
        return False
    ...
    try:
        ...
        logger.error(f"Chatroom open error for {person_id}: {e}")
        return False
        ...
        try:
            ...
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    logger.warning(f"Chat send failed for {person_id} part {i+1}: status {resp.status}")
        except Exception as e:
            logger.error(f"Chat send error for {person_id} part {i+1}: {e}")
...
async def process_account_sequence(
    user_id: int, token_obj: dict, settings: dict,
    session: aiohttp.ClientSession, db_data: dict
):
    ...
    # ------------------------------------------------------------------ #
    # 1. CHECK REQUEST CYCLE (24 h gate)
    # ------------------------------------------------------------------ #
    ...
    # --- FIX: Load spam filter flags for lounge and chatroom ---
    ...
    # --- FIX: Load already-sent sets from DB (mirrors lounge.py / chatroom.py) ---
    lounge_sent_ids: set = (
    ...
    chatroom_history = db_data.get("chatroom_sent", {}).get(token, {})
    ...
    for pid, add_time_str in added_users.items():
        ...
        # ---------------------------------------------------------------- #
        # FIX: Spam-filter gate for LOUNGE (mirrors lounge.py exactly)
        # ---------------------------------------------------------------- #
        if is_lounge_spam_on and pid in lounge_sent_ids:
            update_account_stats(user_id, token, {'lng_f': 1})
            continue

        lounge_result = await _send_lounge_msg(session, token, pid, lounge_msg)

        if lounge_result:
            update_account_stats(user_id, token, {'lng_s': 1})

            # Mark wave so we never send it again (automation_timers)
            await mark_lounge_sent(user_id, token, pid, wave_to_run)

            # FIX: Also save to global sent_records so spam filter stays consistent
            if is_lounge_spam_on:
            ...
            # ---------------------------------------------------------------- #
            # FIX: Spam-filter gate for CHATROOM (mirrors chatroom.py exactly)
            # ---------------------------------------------------------------- #
            # Only skip chatroom if already sent (chatroom is sent once per user
            # across all waves, unlike lounge which tracks per-wave)
            if is_chat_spam_on and pid in chatroom_sent_ids:
                update_account_stats(user_id, token, {'chat_f': 1})
            elif pid in chatroom_history:
                # Already sent via automation_timers record – treat as filtered
                update_account_stats(user_id, token, {'chat_f': 1})
            else:
                chat_result = await _send_chatroom_msg(session, token, pid, chat_msg)

                if chat_result == "DISABLED":
                    # User disabled chat – count as filtered, not failure
                    update_account_stats(user_id, token, {'chat_f': 1})
                elif chat_result:
                    update_account_stats(user_id, token, {'chat_s': 1})

                    # Mark in automation_timers
                    await mark_chatroom_sent(user_id, token, pid)

                    # FIX: Save to global sent_records so spam filter stays consistent
                    if is_chat_spam_on:
    ...
    # ------------------------------------------------------------------ #
    # FIX: Bulk-save new sent IDs to DB (mirrors lounge.py / chatroom.py)
    # ------------------------------------------------------------------ #
...
async def run_requests_task(user_id, token_obj, session):
    ...
    if ui_stats_state[user_id][token]['req_s'] > 0:
        await add_automation_log(
            user_id, f"[{name}] Requests: {ui_stats_state[user_id][token]['req_s']}"
        )
